  ...
  return (
    <div className="bg-primary relative py-28">
      ...
      <div className="container  flex flex-col gap-10 items-center  ">
        <h2 className=" text-black rounded py-4 text-center font-black text-3xl lg:text-6xl">Nos Packs Clé en Main</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {products.map((product) => (
            <div key={product.id} className="bg-black shadow-2xl shadow-black/40 p-6 rounded-3xl text-white flex flex-col gap-4 items-center justify-between">
              ...
              <button onClick={() => handleBuy(product.id)} className="rounded-lg bg-primary hover:bg-black border-2 border-primary hover:text-primary duration-700 w-full text-black py-2 font-bold">{processing?'Traitement en cours... Veuillez patienter.' : 'Acheter Maintenant'}</button>
            </div>
          ))}
        </div>
      </div>
